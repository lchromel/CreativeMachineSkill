...
Setup is part of this skill and works before MCP tools load. Missing tools, an unset token variable, or a 401/403 from the MCP endpoint starts connection setup; pause media generation while completing the applicable setup steps below. The same account-issued personal token authenticates MCP calls and returned media downloads. A status-only reply such as “configuration installed, token missing” is incomplete when a token file or template can be supplied.

1. Check the actual client's connection and the user's designated credential source. Reuse an available account-issued personal token without asking for it again. Our client credential name is `CREATIVE_MACHINE_API_TOKEN`. If an existing client points to `MCP_API_TOKEN`, inspect and align that client setting during setup; do not ask the user to configure Railway or invent a second token.
2. If the Bearer credential is missing, direct the user to [Account → MCP tokens](https://yango-production-60f5.up.railway.app/account#mcp-tokens) to create their personal token after account approval, then **create the personal token file in this turn and include its clickable link before yielding**. The token is shown once, lasts 90 days and can be revoked from that account page. From this skill directory run `python3 scripts/prepare_token_file.py`; use the returned absolute `path` for the file link. The helper creates `~/.config/creative-machine/token.env` without overwriting existing files. If Python is unavailable, copy [the blank template](assets/creative-machine-token.env.example) to that personal location using available file tools. Respect filesystem permissions; filled credentials belong outside repositories and installed skills.
...
For this default trio, the agent selects layouts and copy treatments without an extra design questionnaire. Mention other supported layouts as available alternatives; do not wait for a layout/word/badge choice unless the user requested that choice. This overrides the initial layout-choice workflow below only for the automatic trio. Ask for genuinely missing content, including a missing disclaimer, in one concise message; independent source generation may continue while waiting, but the final render waits for the required copy. Never invent a disclaimer.
...
- For video, first read [video workflows](references/video-workflows.md) and call `get_video_capabilities`. For every new video request, explicitly ask whether the user wants PERF (square 1:1, `outputType="performance"`) or SMM (vertical 9:16, `outputType="smm"`) when format is unstated; there is no format default. Use typed `write_video_prompt` → `submit_video` with identical settings and the returned digest. Collect duration, dynamics, casting, transport, merch and each attachment role explicitly. After success, show the video and offer `edit_video` for logo, packshot, copy, disclaimers and subtitles; apply already-requested editing without another generation.
...
For downloading returned files, use the same account-issued personal Bearer token as the MCP connection. Read it from the user's designated credential source. A 401 means the file is inaccessible to that account or the token is invalid; retain the returned URL and do not rerun generation. The user opens history and editor links by signing into the same account. See [download authorization](references/mcp-setup.md#download-authorization) for origin restrictions and error handling.
...
Upload only user-provided or authorized media. Keep provider credentials and Yandex Disk OAuth in the generator. Video submission uses the same personal token and the account's Video permission. If local credentials are needed, use the user's designated token file first without printing secrets. Distinguish missing MCP access, upstream authentication and insufficient account permission.
