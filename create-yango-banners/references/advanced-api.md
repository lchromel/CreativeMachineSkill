...
- `brand`, `logoVariant`, `layoutType`, `driveServiceName`, `coBrandLogoUrl` carry branding. For Yango set `brand="yango"` and `logoVariant="default"` in both render payload and saved settings; `icon`/`none` are explicit alternatives. A language variant such as `en`/`ar` can select Yandex Go even with the Yango brand and is rejected by MCP. Copy language is separate. Yandex Go B2B uses `brand="yandex-go-b2b"`, frame layout, locales `en/am/kz/uz/uz-ru/kg/sr/ru`, and yellow `#FFFD72`, navy `#081331`, blue `#BFCCFF` or white `#FFFFFF` accents.
